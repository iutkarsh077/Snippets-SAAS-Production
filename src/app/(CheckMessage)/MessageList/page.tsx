        ...
        <div className="w-full   p-4 overflow-y-auto h-full">
          ...
          <div className="mt-4 flex flex-col gap-4">
            {userList.map((user: any) => (
              <div
                key={user.id}
                className="border dark:border-white border-gray-300 rounded-lg shadow-sm hover:shadow-md transition p-4 hover:cursor-pointer"
                onClick={() => {
                  handleUserSelection(user.id), setShowUserList(false);
                }}
              >
                <h3 className="text-lg font-medium text-black dark:text-white">{user.username}</h3>
              </div>
            ))}
          </div>
        </div>
        ...
        >
          {selectedUserId ? (
            messages.map((message: any, index: number) => (
              <div
                key={index}
                className={`border dark:border-black border-gray-300  font-medium dark:text-white text-black rounded-lg p-3 shadow-sm ${
